    ...
    IRefreshTokenValidator refreshTokenValidator) : ICommandHandler<RefreshCommand>
{
    ...
    {
        RefreshTicket? refreshTicket = refreshTokenValidator.UnprotectRefreshToken(command.RefreshToken);
        ...
        IUser? user = await refreshTokenValidator.ValidateSecurityStampAsync(refreshTicket.ClaimsPrincipal);
        if (user == null)
            return Result.Failure(UserErrors.RefreshFailed);
