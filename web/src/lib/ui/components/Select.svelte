<script lang="ts">
	...
	const triggerTextClass = $derived(hasValue ? 'text-gray-800 dark:text-white/90' : 'text-gray-400 dark:text-white/30');
	...
	const itemClass = $derived.by(() =>
		cn(
			...
			hasError ? 'data-selected:text-error-600 dark:data-selected:text-error-400' : 'data-selected:text-brand-600 dark:data-selected:text-brand-400'
		)
	);
	const checkIconClass = $derived.by(() =>
		hasError ? 'shrink-0 text-error-500 dark:text-error-400' : 'shrink-0 text-brand-500 dark:text-brand-400'
	);
	const contentClass =
		...
		{#if multiple}
			...
			>
				<Select.Trigger
					...
					class={triggerClass}
				>
					<span class={cn('block min-w-0 flex-1 truncate', triggerTextClass)} title={selectedLabelTitle}>
						{selectedLabel}
					</span>
